...
window.addEventListener('load', () => {
    ...
    for (let i in LightJsClasses) {
        ...
        styleStr += `.${LightJsClasses[i].replace(/\:/g, '\\:').replace(/\[/g, '\\[').replace(/\]/g, '\\]').replace(/\>/g, '\\>')}`
...
function classEvaluate(el, classes = null) {
    ...
    if (classes.indexOf(el) === -1) {
        ...
        styleStr += `.${el.replace(/\:/g, '\\:').replace(/\[/g, '\\[').replace(/\]/g, '\\]').replace(/\>/g, '\\>')}`
